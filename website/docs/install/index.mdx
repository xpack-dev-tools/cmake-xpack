...
The easiest (and recommended) way to install xPack CMake
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/cmake@3.29.9-1.1 --verbose
`} </CodeBlock>
...
* add <code>@xpack-dev-tools/cmake</code> to
  `package.json` as a
  development dependency; this associates a specific version of xPack CMake with
...
it is also possible to install xPack CMake only globally in
...
In order to access the xPack CMake binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/AppData/Roaming/xPacks/\@pack-dev-tools/cmake/3.29.9-1.1/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/cmake/3.29.9-1.1/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/cmake/3.29.9-1.1/.content/bin:$PATH
`} </CodeBlock>
...
<ul>
<li><code>xpack-cmake-3.29.9-1-win32-x64.zip</code></li>
</ul>
...
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\cmake\\xpack-cmake-3.29.9-1</code>.
...
<ul>
<li><code>xpack-cmake-3.29.9-1-darwin-x64.tar.gz</code></li>
<li><code>xpack-cmake-3.29.9-1-darwin-arm64.tar.gz</code></li>
</ul>
...
example <code>/Users/ilg/Library/xPacks/cmake/xpack-cmake-3.29.9-1</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/cmake
cd ~/Library/xPacks/cmake

tar xvf ~/Downloads/xpack-cmake-3.29.9-1-darwin-x64.tar.gz
chmod -R -w xpack-cmake-3.29.9-1
`} </CodeBlock>
...
<ul>
<li><code>xpack-cmake-3.29.9-1-linux-x64.tar.gz</code></li>
<li><code>xpack-cmake-3.29.9-1-linux-arm64.tar.gz</code></li>
<li><code>xpack-cmake-3.29.9-1-linux-arm.tar.gz</code></li>
</ul>
...
example <code>/home/ilg/.local/xPacks/cmake/xpack-cmake-3.29.9-1</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/cmake
cd ~/.local/xPacks/cmake

tar xvf ~/Downloads/xpack-cmake-3.29.9-1-linux-x64.tar.gz
chmod -R -w xpack-cmake-3.29.9-1
`} </CodeBlock>
